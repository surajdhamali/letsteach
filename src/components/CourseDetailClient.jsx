...
export default function CourseDetailClient({ course, related, discount }) {
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2">
            ...
            <motion.span
              ...
              className={`inline-block text-xs font-semibold px-3 py-1 rounded-full mb-4 ${course.categoryColor}`}
            >
            ...
            <motion.h1
              ...
              className="font-display text-3xl md:text-4xl font-semibold text-white tracking-tight leading-tight mb-4"
            >
              {course.title}
            </motion.h1>
            ...
            <motion.p
              ...
              className="text-zinc-400 text-base leading-relaxed mb-6 max-w-2xl"
            >
              {course.description}
            </motion.p>
          ...
          >
            <div className="bg-white rounded-2xl p-6 sticky top-20">
              <div className={`${course.image ? "" : course.thumbBg} h-40 rounded-xl flex items-center justify-center mb-5 overflow-hidden relative`}>
                {course.image ? (
                  <Image src={course.image} alt={course.imageAlt || course.title} fill sizes="400px" className="object-cover rounded-xl" />
                ) : (
              ...
              <div className="mb-4">
                ...
                <span className="ml-2 text-sm font-semibold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full">{discount}% off</span>
              </div>
              <p className="text-xs text-rose-500 font-medium mb-4">⏰ 2 days left at this price</p>
              <Link href="/signup" className="block w-full text-center text-sm font-semibold bg-amber-600 text-white py-3 rounded-full hover:bg-amber-500 transition-colors mb-3 hover:scale-[1.02] active:scale-95">
                Enroll now
              </Link>
              ...
              <div className="border-t border-zinc-100 pt-4 space-y-2">
                {[["∞","Lifetime access"],["📱","Access on all devices"],["◎",`${course.lectures} lectures · ${course.duration}`],["✦","Certificate of completion"]].map(([icon, text]) => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {course.highlights.map((h, i) => (
                <motion.div
                  ...
                  className="flex items-start gap-3 bg-zinc-50 border border-zinc-100 rounded-xl p-3"
                >
              ...
              {course.curriculum.map((section, si) => (
                ...
                >
                  <button
                    ...
                    className="w-full bg-zinc-50 px-4 py-3 flex items-center justify-between hover:bg-zinc-100 transition-colors"
                  >
                          ...
                          {section.lessons.map((lesson, li) => (
                            ...
                            >
                              <div className="w-5 h-5 rounded-full bg-zinc-100 flex items-center justify-center flex-shrink-0">
                                <svg className="w-2.5 h-2.5 text-zinc-400" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>
                              </div>
      ...
      {related.length > 0 && (
        <div className="bg-zinc-50 border-t border-zinc-100 px-6 py-12">
          <div className="max-w-6xl mx-auto">
            <FadeUp>
              <h2 className="font-display text-2xl font-semibold tracking-tight mb-6">More in {course.category}</h2>
            </FadeUp>
